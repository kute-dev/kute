...
- **Joins render before you touch anything:** a label a Service selector matches carries an inline yellow `⚠ selector · svc/aim-worker`; editing that key requires the inline y/N even though metadata edits are otherwise reversible, and the confirm names the blast radius (`detaches 4 pods from svc/aim-worker`). Deployment selector labels are immutable server-side — kute says so up front instead of letting the apply bounce off the API server.
...
- **The panel stays open through a confirm, not the generic table-with-overlay.** Unlike 8b's own delete confirm — which closes whatever was showing and drops a plain y/N over the bare table — a join-locked edit or a removal's inline y/N renders *inside the still-open editor*: the row under confirmation shows its about-to-apply state (`was aim-worker · aim-worker2` for an edit, a red `remove · y/N` note for a removal) and the panel's own will-run strip prints the exact command (plus the `detaches N pods from svc/X` warning for a join, ahead of the command) — the keybar's own note stays to the short join-warning form alone, since duplicating the full line there routinely overruns its width and gets silently dropped. Cancelling (`n`/`esc`) reverts an in-flight edit's buffer and returns to navigation without closing the panel.
