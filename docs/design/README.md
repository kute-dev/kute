...
- **Same confirm → execute → refresh → show result → remain on screen contract as 26a — with the value never shown either way:** applying never closes the editor — the grid re-fetches from the Secret afterward (masked rows stay masked, the new key joins them immediately re-masked, the count ticks `3 keys → 4`), and the will-run strip becomes an inline result naming the key alone (`added SMTP_PASSWORD` / `removed SMTP_PASSWORD` — never `=value`, success or not, per this screen's own no-leak rule above). A rejected apply leaves the input row's typed value exactly as entered (still revealed/masked per its own `x` state) with the server's error shown; `esc` is what actually leaves, and still zeroes the buffer.
