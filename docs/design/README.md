...
- **The condition message renders verbatim, under its row.** Every Flux CRD declares exactly `Ready`, `Status` and `Age` (sources add `URL`), where the declared `Status` column *is* the Ready condition's message — prose like `health check failed after 2m0s: Deployment/aim-stage/aim-worker status: 'InProgress'`, which a table cell can only ellipsize into uselessness. It becomes a full-width continuation line under the row instead: muted, never selectable, never paraphrased. A healthy row gets none (its message only repeats the REVISION cell), and neither does a suspended one (the READY cell already says the word).
- REVISION and SOURCE are derived — Flux declares neither, and §14a's "never guesses a column that isn't there" governs the *generic* descriptor, not a curated kind (§18a's Helm columns and §23b's ATTACHED are the same licence). REVISION collapses `main@sha1:efd398be…` to `main@efd398b`; a source's own artifact revision can be a bare 71-character digest, which shortens the same way. SOURCE reads `git/aim-config`, and `–` on a source kind, which *is* the source.
