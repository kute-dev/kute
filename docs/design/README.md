...
- Not a new screen — the shared list skeleton with CronJob's own columns: status glyph · NAME · SCHEDULE · SUSP · ACT · LAST RUN · NEXT. CronJobs stay in **fixed name order always**: numeric manual-sort keys (2a) are ignored for this kind and the sort arrow stays hidden, the same treatment 6b's grouped view gets — a suspended cronjob is usually suspended *by you, this morning*, and needs to be found where you left it, not sunk to the bottom by a health-first default.
...
- **LAST RUN carries the newest *terminal* associated Job's outcome** (✓/✕) — not the currently active one, so an active row can read `2m ago ✓` in LAST RUN while ACT reads `1`, exactly as 36a's mockup shows. Suspended rows render dim with a struck-through NAME, amber `SUSP yes`, and `NEXT —`, but stay in name order. A selected failed row expands one inline reason line beneath itself — the deepest Job condition message, verbatim, visible only while that row is selected; non-selected failures stay one line.
