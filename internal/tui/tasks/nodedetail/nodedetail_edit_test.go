...
func TestEditProdShowsInlinePromptAndLaunchesOnY(t *testing.T) {
	...
	m := New(Config{Session: sess, Lister: lister, NodeName: "node-a"})
	m.SetSize(120, 40)
	m = step(t, m, m.Init()())
	...
	kb := m.Keybar()
	if kb.RightNote == "" || !strings.Contains(kb.RightNote, "node-a") {
...
// mirrors exec's/node-shell's own feedback contract.
