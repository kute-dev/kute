...
### 36a — CronJobs (extends 9a — `ctrl-r` run now · `s` suspend/resume · `S` edit schedule)
- **Not a new screen** — the pods skeleton with CronJob's own columns: status glyph · NAME · SCHEDULE · NEXT RUN · SUSPEND · ACTIVE · AGE. NEXT RUN reads `in 4m`/`in 2h`/`in 3d` off the schedule string (robfig/cron/v3 — the same library the real CronJob controller parses schedules with server-side, including `spec.timeZone`), anchored on now rather than `status.lastScheduleTime`; `—` for a suspended row or one whose schedule fails to parse.
- Keys: `↵` = the CronJob's own pods, skipping the intermediate Job it spawns (same name-prefix fuzzy filter 9a's own `↵` uses — a CronJob spawns a Job named `<cronjob>-<unixtime>`, whose pods are `<cronjob>-<unixtime>-<random>`, still prefixed by the CronJob's name) · `ctrl-r` run now (`kubectl create job --from=cronjob/…`, a brand-new standalone Job — inline `y/N`, non-destructive so it never escalates past that, same reasoning as Jobs' own retry) · `s` suspend/resume (`spec.suspend` — TierNone, reversible and immediate: unlike a Job's own `s`, this only pauses *future* scheduling, nothing already running is touched — the same distinction §30a draws between FluxSuspend and JobSuspend) · `S` edit schedule (inline text buffer, client-side cron-parse validation before apply, PROD escalates to inline `y/N` — the same nominal-TierNone/PROD-escalates shape 24a's set-image uses) · `y` yaml · `g` goto. Keybar pill `CRONJOBS`.
