...
- **Header bar** (bg `#0e0e15`, bottom border `#26263a`): app wordmark `❯kute` — the `❯` mark in brand green `#5ddba4` (`#12855a` light), `kute` itself in the plain bright text token `#f0f0fa` (`#14141c` light, bold) `│` `c` context `microk8s-cluster` (dim `#676780`) `›` `n` namespace `nva-stage` (`#9a9ab2`, one step brighter than context — not the accent purple its own `n` key uses) `›` `g` kind `Pods` (bold `#f0f0fa`). The `c`/`n`/`g` prefixes live on the breadcrumb segment each one changes, colored the same purple as keybar keys (`#a78bfa`, `#6b46d9` light) — no separate hint text. Right-aligned: `sync 2s` (dim) and connection status `● connected · 12ms` (green `#34d17b`).
