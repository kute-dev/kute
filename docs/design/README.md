...
- **Namespace-scoped hint:** when a namespace is known (a kind/namespace is already selected) and the session isn't already running `--namespace-scoped`, one more line names it — `kute --namespace-scoped=<ns> may see more, if that namespace grants it` — since a cluster-wide 403 is exactly the shape a namespace-bound identity (a `Role`, not a `ClusterRole`) hits on a plain launch (`docs/plans/namespace-scoped-final-plan.md`). Omitted for a session already scoped, or before any namespace is known.
