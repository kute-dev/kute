...
- `↵` = objects in the release (filtered tables, 9a's recipe) · `h` history (16b's rail idiom) · `v` values in the YAML viewer, read-only. Rollback inherits 8b friction. **No install/upgrade-from-repo — deliberately out of scope (CI's job); detecting that an upgrade exists is not performing one.** Keybar pill `HELM`.
