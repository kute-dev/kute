...
// covered here is poddetail's hosting glue: 'm' opens it on the loaded pod,
...
// focused buffer through poddetail's own pasteTarget resolver.
func TestMetaKeyOpensPanelOnPod(t *testing.T) {
	p := crashLoopPod("api-0", "default", "node-1")
	...
	body := m.Body(120, 30)
	if !strings.Contains(plain(body), "LABELS · 1") || !strings.Contains(plain(body), "app=") {
	...
	if !strings.Contains(plain(body), "api-0") {
		t.Error("panel body should keep the pod's own title line above the grid")
	}
