...
### 30b — Flux tree (deferred, not rejected)
- One screen grouped by source: each GitRepository/HelmRepository with the Kustomizations and HelmReleases it drives nested under it. Flux's failure mode is chain-shaped (source fetches → reconciler applies → workload rolls out) and no kubectl view shows the chain — the same both-ways-join argument §23b makes for Gateway API.
- Deferred because it is a bespoke screen that cuts against "kinds are data, not code" (30a is the registry baseline that ships either way), and because its headline drift delta (`−9`, "source is 9 commits ahead") is unbuildable without git access, per 31a above.
