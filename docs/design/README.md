...
- **Three columns, ~6 rows each:** column 1 = current view's keys (swaps per view: PODS VIEW, LOGS…), columns 2–3 fixed = SCOPE (`g n c a / ↵ toggles last`) and GLOBAL (`↑↓ jk`, `1-9 sort column`, `esc`, `p pause sync`, `r reconnect`, `?`, `ctrl+q`). Column headings `#c4b5fd` uppercase over `#1c1c2c` rule.
...
- **Generic navigation stays out of task keybars:** plain `↵ open` and `esc back` are taught once in the help overlay's GLOBAL column and omitted from every screen's bottom band. Screen-specific Enter/Escape actions (`↵ pods`, `↵ edit`, `↵ open object`, `esc clear`, `esc cancel`, …) still render where they add information; per-screen key lists elsewhere in this document describe behavior subject to this keybar rule.
